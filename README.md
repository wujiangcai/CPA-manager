...
- 大 JSON 一键导入：选择超过 10MB 的账号 JSON/JSONL，自动拆成单账号小 JSON，导入 CPA 后检测存活，并输出 alive / dead 两个文件夹。
...
如果配置了 `proxy_url`，系统会在检测前先通过代理访问 `proxy_check_url`；代理不通会直接中止检测，避免把账号误判为死亡。
